...
第一段階として、推薦スコア計算・理由生成・表示スコア正規化は `recommendation-engine.js` に切り出し済みです。`app.js` は classic script のまま維持し、推薦タグ利用時に `import('./recommendation-engine.js?...')` で動的に読み込みます。これにより、推薦エンジンの読み込みに失敗しても、地図・検索・フィルタ本体は動き続けます。なお、`recommendation-engine.js` 自体は小容量のためService WorkerのAPP_SHELLに含め、PWA再訪時・オフライン時の推薦UI耐性を優先します。2MB超の `recommendation_tags.json` は初期キャッシュせず、利用時にランタイムキャッシュします。
...
- `footerAttribution` のような固定定義由来のHTMLだけを例外的に許容し、外部入力・店舗データ・外部シグナル由来の文字列は直接入れない
...
`style-src` の `'unsafe-inline'` は、Leaflet、動的UI、既存スタイル更新との互換性を優先して現時点では維持します。外す場合は、インラインstyleの棚卸し、MapLibre/Leafletの挙動確認、iOS WKWebView確認を同時に行います。

JSON-LDはデータ読み込み後に `textContent` で注入します。meta CSPでは固定nonceを許可していますが、これは静的ホスティング上の互換策であり、任意スクリプトを許可する意図ではありません。構造化データ以外の動的script注入には使わない方針です。
