...
現行実装では、体験の近さを出しやすくするため、ジャンル・地域・選出履歴のような事実タグを控えめにし、店の系統・麺質・料理・雰囲気タグを強めています。

| タグ種別 | 係数 | 意味 |
| --- | ---: | --- |
| `genre.*` | 0.9 | 同ジャンルの近さ。強すぎると単調になるため控えめ |
| `style.*` | 1.45 | 店舗タイプ・流派の近さ |
| `texture.*` | 1.45 | 食感・味覚の近さ |
| `dish.*` | 1.35 | 看板料理の近さ |
| `lineage.*` | 1.3 | そば系譜などの近さ |
| `scene.*` | 1.08 | 利用シーンの近さ |
| `mood.*` | 1.15 | 雰囲気の近さ |
| `pref.*` | 0.42 | 都道府県の近さ。理由表示では原則使わない |
| `macro_area.*` | 0.32 | 広域の近さ。理由表示では原則使わない |
| `selection.*` | 0.34 | 名店性・選出傾向の近さ。補助扱い |
| `region.*` | 0.25 | EAST/WEST/KAGAWAの近さ。補助扱い |
...
| `distanceFactor` | 「最寄りで探す」では50%の比重で使う |
...
を強めます。`genre.*`, `pref.*`, `region.*`, `selection.*` は補助に留めます。

### 最寄りで探す

地理的に行きやすい候補を優先するモードです。

- 選択店舗からの距離を50%の比重で補正する。
...
2. `style.*`, `texture.*`, `dish.*`, `mood.*`, `scene.*`, `lineage.*` を優先する。
3. `genre.*`, `pref.*`, `region.*`, `macro_area.*` は原則として理由文に出さない。
...
```text
weight × confidence >= 0.28
```

例:

```text
「讃岐系の傾向」「コシ重視寄り」「目的地として訪ねる店」が近い候補です。
```

```text
「石臼・玄蕎麦寄り」「落ち着いた食事向き」「酒・蕎麦前寄り」が近い候補です。
```
...
- 各候補に理由タグを最大3個表示する。
- 通常推薦では閉店・移転を除外する。
- 候補自身が「行きたい」の場合は少し加点する。
- 訪問済みは減点しない。
- 「行きたい」に入れた店舗群から、ユーザーの嗜好タグを軽く反映する。

## ゴールデンセット評価

`data/recommendation_golden_set.json` には、推薦品質を確認するための代表ケースを置いています。現時点では36ケースです。

これは厳密な正解データではありません。店舗推薦には主観が含まれるため、CIで合否判定するのではなく、スコア調整やタグ生成ルール変更の前後で「期待候補が上位に来るか」「理由文が自然か」「違和感のあるカテゴリが混ざっていないか」を見るためのレビュー用データです。

主な確認観点:

- 期待候補がTop3またはTop9に入るか。
- `genre`, `pref`, `region`, `selection` だけで推薦されていないか。
- 理由文が `style`, `texture`, `dish`, `mood`, `scene`, `lineage` を中心に説明できているか。
- うどんケースにそば、そばケースにうどんが過剰に混ざっていないか。
- 「新しい発見」では同質すぎる候補だけになっていないか。
- 武蔵野うどん、吉田うどん、稲庭、博多・福岡、香川、東京そば、藪系、翁系、信州、北関東、中部・北陸など、複数の文脈で大きく崩れていないか。
...
タグ類似度だけでは、地域・選出回数などの事実タグに寄りすぎたり、逆に「この店が好きならこの店も」という横の連想が弱くなることがあります。

その補助として、`data/recommendation_tags.json` には `affinityGroups` を持たせています。これは店舗の事実説明ではなく、AI推定による探索文脈です。
...
実行時は、起点店舗と候補店舗が同じ `affinityGroups` に属しており、かつ推薦モードが一致する場合にだけ、小さな加点を行います。現時点では最大16%までに制限しています。

この仕組みの意図:

- タグ辞書を過剰に増やさず、店同士の文脈を補助する。
- 推薦を固定順位にしない。
- 事実タグとAI推定の相性判断を分離する。
- ゴールデンセットで結果を見ながら、対象グループや補正量を調整できるようにする。
