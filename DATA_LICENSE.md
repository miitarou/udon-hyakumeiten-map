...
本サイトは個人が非商用で公開している非公式ファンツールです。
店舗データは、食べログ等の各情報提供元の権利・利用条件の影響を受ける可能性があるため、JSONデータの再利用・再配布は推奨しません。
再利用する場合は、利用者自身の責任で各情報提供元の条件を確認してください。

本リポジトリでは、食べログのレビュー本文、点数、口コミ、写真等は取り扱いません。
店舗名、選出年度、住所、座標、URL、閉店情報等の参考情報のみを整理対象としています。

店舗情報の削除・修正依頼がある場合は、GitHub Issues またはリポジトリ管理者への連絡によりお知らせください。
確認でき次第、必要に応じて修正・削除します。
